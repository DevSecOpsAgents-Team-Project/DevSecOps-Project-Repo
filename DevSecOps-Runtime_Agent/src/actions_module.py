...
def _action_result_detail(result) -> str:
    if not isinstance(result, dict):
        return "결과 없음"
    ...
    return _action_result_status(result)


def summarize_mitigation_steps(steps):
    """
    steps: [(label, result_dict|None), ...]
    Returns: (overall_status, slack_message)
    overall_status: SUCCESS | PARTIAL_SUCCESS | FAILED | SKIPPED
    """
    if not steps:
        return "SKIPPED", "⏭️ *L1 조치 스킵* — 실행된 액션이 없습니다."

    success = failed = skipped = 0
    lines = []
    for label, result in steps:
        status = _action_result_status(result)
        detail = _action_result_detail(result)
...
def notify_mitigation_outcome(
    ...
    """L1 Step2 조치 결과를 Slack으로 전송."""
    overall, summary = summarize_mitigation_steps(steps)
    message = (
        f"{summary}\n\n"
